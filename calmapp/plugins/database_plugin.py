...
from .database_plugin_config import DatabaseConfig, DatabaseMode
...
class DatabasePlugin(Plugin):
    ...
    def __init__(self, app: "App", config: "AppConfig", **kwargs):
        super().__init__(app, config, **kwargs)

        self.mode = config.database.mode

        conn_str = config.database.conn_str.get_secret_value()
        if conn_str:
            self._db = self._connect_db(conn_str, config.database.name, "main_database")
        ...
        if private_url:
            self._private_db = self._connect_db(
                private_url, config.database.private_name, "private_database"
            )
            self.logger.info(
        ...
        if public_url:
            self._public_db = self._connect_db(
                public_url, config.database.public_name, "public_database"
            )
            self.logger.info(
